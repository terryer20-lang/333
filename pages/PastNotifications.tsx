...
const PastNotifications: React.FC = () => {
  ...
  const notifications: Notification[] = [
    { id: '1', category: 'alert', date: '2026-03-15', titleKey: 'item_1_title', descKey: 'item_1_desc', readTime: '3 min' },
    { id: '2', category: 'news', date: '2026-02-28', titleKey: 'item_2_title', descKey: 'item_2_desc', readTime: '5 min' },
    { id: '3', category: 'activity', date: '2026-02-10', titleKey: 'item_3_title', descKey: 'item_3_desc', readTime: '2 min' },
    { id: '4', category: 'alert', date: '2025-12-24', titleKey: 'item_4_title', descKey: 'item_4_desc', readTime: '4 min' },
    { id: '5', category: 'news', date: '2025-12-20', titleKey: 'item_5_title', descKey: 'item_5_desc', readTime: '1 min' },
    { id: '6', category: 'alert', date: '2025-11-15', titleKey: 'item_6_title', descKey: 'item_6_desc', readTime: '3 min' },
  ];
  ...
  // Heatmap Data Calculation (Simple frequency by month)
  const heatmapData = useMemo(() => {
     ...
     return Object.entries(counts).sort((a, b) => b[0].localeCompare(a[0])).slice(0, 5); // Last 5 active months
  }, []);
    ...
    <div className="min-h-screen bg-gray-50 pb-24 font-sans">
      ...
      <div className="relative pt-28 pb-10 px-6 bg-slate-900 text-white rounded-b-[2.5rem] shadow-2xl overflow-hidden">
        ...
        <div className="relative z-10">
          <h1 className="text-3xl font-black mb-1">{t('notifications_page.hero_title')}</h1>
          <p className="text-sm text-gray-400 mb-6">{t('notifications_page.hero_subtitle')}</p>
          ...
          <div className="grid grid-cols-3 gap-3 mb-6">
             ...
             <div className="bg-white/10 backdrop-blur rounded-xl p-3 border border-white/5 text-center">
                <div className="text-2xl font-bold text-blue-400">{notifications.filter(n => n.category === 'news').length}</div>
                <div className="text-[10px] text-gray-400">{t('notifications_page.stats_tips')}</div>
             </div>
          </div>

          {/* Activity Heatmap (Visual Fluff) */}
          <div className="bg-slate-800/50 rounded-xl p-3 border border-white/5">
             <div className="text-[10px] text-gray-400 uppercase tracking-wider mb-2 flex justify-between">
                <span>{t('notifications_page.heatmap_title')}</span>
                <span>Last 5 Months</span>
             </div>
             <div className="flex gap-2 justify-between items-end h-8">
                {heatmapData.map(([month, count]) => (
                   <div key={month} className="flex-1 flex flex-col gap-1 items-center group">
                      <div 
                        className="w-full bg-brand-green rounded-sm opacity-80 group-hover:opacity-100 transition-opacity" 
                        style={{ height: `${count * 8}px`, minHeight: '4px' }}
                      ></div>
                      <span className="text-[8px] text-gray-500 font-mono">{month.split('-')[1]}</span>
                   </div>
                ))}
             </div>
          </div>
                     ...
                     >
                        ...
                        <div className="flex justify-between items-center border-t border-gray-50 pt-2">
                           <div className="text-[10px] text-gray-400 flex items-center gap-1">
                              <span>⏱️</span> {item.readTime} read
                           </div>
